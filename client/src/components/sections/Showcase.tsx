      ...
      <Container>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {showcaseItems.map((item, i) => (
            <MotionWrapper key={i} delay={i * 0.15} className="group relative rounded-[40px] overflow-hidden aspect-[3/4] card-lift bg-white shadow-xl border border-black/5">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-8">
                <span className="text-2.5 uppercase tracking-[0.2em] text-white/80 font-bold md:drop-shadow-md">{item.tag}</span>
                <h3 className="text-2xl font-heading font-semibold text-white mt-1 md:drop-shadow-lg tracking-tight">{item.label}</h3>
              </div>
              
              {/* Hover CTA */}
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/60 md:bg-black/20 md:backdrop-blur-0.5">
                <a
