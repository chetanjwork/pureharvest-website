    ...
    <Section className="bg-brand-primary text-brand-accent pt-10 lg:pt-10 pb-20 lg:pb-24" id="process">
      <Container>
        ...
        <div className="flex flex-col items-center justify-center -mt-10 mb-16 pointer-events-none">
          <div className="w-px h-20 bg-linear-to-b from-brand-accent/20 via-brand-accent/5 to-transparent" />
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-10 relative">
          ...
          <div className="hidden lg:block absolute top-10 left-[12.5%] right-[12.5%] h-px bg-black/4 z-0" />
            ...
            return (
              <MotionWrapper key={index} delay={index * 0.1} className="relative group z-10">
                <motion.div
                  ...
                  className="flex flex-col h-full bg-white/95 md:bg-white/50 md:backdrop-blur-sm rounded-4xl p-8 border border-black/3 shadow-[0_8px_32pxrgba(0,0,0,0.02)] hover:shadow-[0_16px_48pxrgba(0,0,0,0.06)] hover:bg-white transition-all duration-300 cursor-default"
                >
                  {/* Step icon circle */}
                  <div className="w-16 h-16 rounded-[20px] bg-white shadow-sm border border-black/4 flex items-center justify-center mb-8 relative group-hover:scale-105 transition-transform duration-500 ease-out">
                    <Icon size={24} strokeWidth={1.5} className="text-brand-secondary" />
                    {/* Step number badge */}
                    <span className="absolute -top-2 -right-2 w-6 h-6 bg-brand-accent rounded-full text-brand-primary text-2.5 font-bold flex items-center justify-center shadow-md">
                      {index + 1}
                    </span>
                  </div>

                  <h2 className="text-4.75 font-semibold mb-4 text-brand-accent tracking-tight">
                    {step.title}
                  </h2>
                  <p className="text-brand-accent/60 leading-relaxed text-3.75 font-medium">
                    {step.description}
                  </p>
                </motion.div>
              </MotionWrapper>
        ...
        <MotionWrapper delay={0.5} className="mt-24 text-center">
          <a
            ...
            className="inline-flex items-center gap-3 bg-brand-accent text-brand-primary px-10 py-5 rounded-full text-3.25 font-bold uppercase tracking-widest shadow-xl hover:scale-105 transition-transform duration-300"
          >
            Start Your Order
            <motion.span
              animate={{ x: [0, 4, 0] }}
