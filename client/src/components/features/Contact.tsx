  ...
  return (
    <Section className="bg-brand-primary text-brand-accent py-24" id="contact">
      <Container className="grid lg:grid-cols-2 gap-20 items-start">
        ...
        <div className="bg-[#F8F9FA] md:bg-black/2 border border-black/5 md:backdrop-blur-xl p-10 md:p-12 rounded-[40px] overflow-hidden">
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-2.5 uppercase tracking-widest font-bold text-brand-accent/60 ml-1">Full Name</label>
                    <input disabled={status === 'loading'} required type="text" placeholder="Arjun Sharma" className="w-full px-6 py-4 rounded-2xl bg-[#F8F9FA] md:bg-black/3 border border-black/10 text-brand-accent placeholder:text-brand-accent/40 focus:outline-none focus:border-brand-accent/40 transition-all font-medium disabled:opacity-50" value={formData.name} onChange={(e) => {
                      const cleanValue = e.target.value.replace(/[^a-zA-Z\s]/g, '');
                      setFormData({ ...formData, name: cleanValue });
                    }} />
                  </div>
                  <div className="space-y-2">
                    <label className="text-2.5 uppercase tracking-widest font-bold text-brand-accent/60 ml-1">Email</label>
                    <input disabled={status === 'loading'} required type="email" placeholder="arjun@enterprise.com" className="w-full px-6 py-4 rounded-2xl bg-[#F8F9FA] md:bg-black/3 border border-black/10 text-brand-accent placeholder:text-brand-accent/40 focus:outline-none focus:border-brand-accent/40 transition-all font-medium disabled:opacity-50" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} />
                  </div>
                </div>
                
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-2.5 uppercase tracking-widest font-bold text-brand-accent/60 ml-1">Mobile Number</label>
                    <div className="relative">
                      <span className="absolute left-6 top-1/2 -translate-y-1/2 text-brand-accent/40 font-medium">+91</span>
                      <input disabled={status === 'loading'} required type="tel" maxLength={10} placeholder="9876543210" className={`w-full pl-16 pr-6 py-4 rounded-2xl bg-[#F8F9FA] md:bg-black/3 border ${phoneError ? 'border-red-500' : 'border-black/10'} text-brand-accent placeholder:text-brand-accent/40 focus:outline-none focus:border-brand-accent/40 transition-all font-medium disabled:opacity-50`} value={formData.phone} onChange={(e) => {
                  ...
                  <div className="space-y-2">
                    <label className="text-2.5 uppercase tracking-widest font-bold text-brand-accent/60 ml-1">Company</label>
                    <input disabled={status === 'loading'} type="text" placeholder="Organization Name" className="w-full px-6 py-4 rounded-2xl bg-[#F8F9FA] md:bg-black/3 border border-black/10 text-brand-accent placeholder:text-brand-accent/40 focus:outline-none focus:border-brand-accent/40 transition-all font-medium disabled:opacity-50" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })} />
                  </div>
                </div>

                <div className="space-y-2">
                  <label className="text-2.5 uppercase tracking-widest font-bold text-brand-accent/60 ml-1">Message</label>
                  <textarea disabled={status === 'loading'} required rows={4} placeholder="Describe your requirements..." className="w-full px-6 py-4 rounded-2xl bg-[#F8F9FA] md:bg-black/3 border border-black/10 text-brand-accent placeholder:text-brand-accent/40 focus:outline-none focus:border-brand-accent/40 transition-all font-medium resize-none disabled:opacity-50" value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} />
                </div>
